...
interface Psychologist extends AdminUser {
  experience: number;
  degree: string;
}

interface RawPsychologistRequest {
  _id: string;
...
interface PsychologistRequestsTabProps {
  isActive: boolean;
}

// Transform function
const transformPsychologistRequest = (
  raw: RawPsychologistRequest
): PsychologistRequest => ({
  id: raw._id,
  subscriberId: raw.subscriber_id,
  requestedDate: raw.preferred_date,
  status: raw.status,
  psychologistId: raw.psychologist_id || undefined,
  ...
  subscriber: raw.subscriber
    ...
    : undefined,
  psychologist: raw.psychologist
    ? {
        name: raw.psychologist.name,
        phone: raw.psychologist.phone,
        experience: raw.psychologist.experience,
        degree: raw.psychologist.degree,
      }
    : undefined,
});

export default function PsychologistRequestsTab({
  isActive,
}: PsychologistRequestsTabProps) {
  ...
  // Queries
  const { data: rawPsychologistRequests = [], isLoading: isLoadingRequests } =
    useQuery<RawPsychologistRequest[]>({
      queryKey: [API_ENDPOINTS.VISIT_REQUESTS_PSYCHOLOGIST],
      staleTime: 30000,
      gcTime: 300000,
  ...
  const {
    ...
    queryKey: [API_ENDPOINTS.VISIT_REQUESTS_PSYCHOLOGISTS],
    staleTime: 30000,
    gcTime: 300000,
    refetchOnWindowFocus: false,
    enabled: isActive,
  ...
  const psychologistRequests = useMemo(
    () => rawPsychologistRequests.map(transformPsychologistRequest),
    [rawPsychologistRequests]
  );
  ...
  const assignPsychologistMutation = useMutation({
    mutationFn: async ({
      ...
      return apiRequest(
        "PATCH",
        API_ENDPOINTS.VISIT_REQUESTS_PSYCHOLOGIST_ASSIGN(requestId),
        {
    ...
    onSuccess: () => {
      ...
      toast({
        title: "Success",
        description: "Psychologist assigned successfully",
        duration: 3000,
      });
    ...
    onError: (error: Error) => {
      toast({
        title: "Error",
        description: error.message || "Failed to assign psychologist",
        variant: "destructive",
        duration: 4000,
      });
    },
  ...
  const handleAssignPsychologist = useCallback(() => {
    if (selectedRequestId && selectedPsychologist && appointmentDateTime) {
  ...
  const canAssignPsychologist = (status: string) => {
    return ["pending", "assigned", "cancelled"].includes(status);
  };

  const getAssignButtonText = (status: string) => {
    if (status === "pending") {
      return "Assign Psychologist & Schedule";
    } else if (status === "cancelled") {
      return "Reactivate & Assign Psychologist";
    } else {
      return "Reassign Psychologist";
    }
  };

  const getDialogTitle = (status: string) => {
    if (status === "pending") {
      return "Assign Psychologist & Schedule Appointment";
    } else if (status === "cancelled") {
      return "Reactivate & Assign Psychologist";
    } else {
      return "Reassign Psychologist";
  ...
  const requestStats = useMemo(() => {
    return {
      total: psychologistRequests.length,
      pending: psychologistRequests.filter((r) => r.status === "pending")
        .length,
      accepted: psychologistRequests.filter((r) => r.status === "accepted")
        .length,
      assigned: psychologistRequests.filter((r) => r.status === "assigned")
        .length,
      in_progress: psychologistRequests.filter(
        (r) => r.status === "in_progress"
      ).length,
      completed: psychologistRequests.filter((r) => r.status === "completed")
        .length,
      cancelled: psychologistRequests.filter((r) => r.status === "cancelled")
        .length,
    };
  }, [psychologistRequests]);

  // Filter requests
  const filteredRequests = useMemo(() => {
    let filtered = psychologistRequests;
    ...
    if (searchQuery.trim()) {
      ...
      filtered = filtered.filter((request) => {
        ...
        const hasPsychologistMatch = request.psychologist?.name
          ?.toLowerCase()
          .includes(query);
        return (
          hasSubscriberMatch ||
          hasPsychologistMatch ||
  ...
  }, [psychologistRequests, filterStatus, searchQuery]);
  ...
  const renderRequestCard = (request: PsychologistRequest, index: number) => {
    ...
    return (
      ...
      >
        ...
        <CardContent className="space-y-3 sm:space-y-4 px-3 sm:px-6 pb-3 sm:pb-6">
          ...
          {canAssignPsychologist(request.status) && (
                ...
                <DialogContent className="sm:max-w-[500px]">
                  <DialogHeader>
                    ...
                    <DialogDescription className="text-xs sm:text-sm">
                      {request.status === "pending"
                        ? "Select a psychologist and schedule appointment time for request"
                        : "Change the assigned psychologist for this request"}
                      <div className="mt-2">
                      ...
                      {request.psychologist && (
                        <div className="mt-2 p-2 bg-amber-50 dark:bg-amber-900/20 rounded text-xs">
                          <span className="text-amber-700 dark:text-amber-300">
                            Currently assigned to: {request.psychologist.name}
                          </span>
                        </div>
                      )}
                    </DialogDescription>
                  ...
                  <div className="space-y-4 mt-4">
                      ...
                      {isLoadingPsychologists ? (
                        ...
                        <div className="text-center py-8 space-y-2">
                          ...
                          <p className="text-sm text-muted-foreground">
                            No approved psychologists available
                          </p>
                        </div>
                      ) : (
                        <Select
                        ...
                        >
                          <SelectTrigger className="w-full h-9 sm:h-10 text-xs sm:text-sm">
                            <SelectValue placeholder="Choose a psychologist" />
                          </SelectTrigger>
                          <SelectContent>
                              ...
                              >
                                <div className="flex flex-col gap-1">
                                  ...
                                  <span className="text-xs text-muted-foreground">
                                    ...
                                    {psychologist.phone} • {psychologist.degree}
                                    {psychologist.subscriptionStatus ===
                                      "active" && (
                    ...
                    <div className="space-y-2">
                      ...
                      <p className="text-xs text-muted-foreground">
                        Select the date and time for the psychologist
                        appointment
                      </p>
                    </div>
                    <Button
                      ...
                      disabled={
                        assignPsychologistMutation.isPending ||
                        !selectedPsychologist ||
                        !appointmentDateTime
                      }
                    >
    ...
    <div className="space-y-4 sm:space-y-6">
        ...
        <CardContent className="p-3 sm:p-4 md:p-6">
          <div className="space-y-3 sm:space-y-4">
            <div className="relative">
              ...
              <Input
                placeholder="Search by subscriber name, phone, psychologist, notes, or ID..."
                value={searchQuery}
      ...
      <div className="flex items-center justify-between">
        <h3 className="text-sm sm:text-base md:text-lg font-semibold text-gray-900 dark:text-white truncate">
          {filterStatus === "all"
            ? "All Psychologist Visit Requests"
              ...
              <div>
                <h3 className="text-base sm:text-lg font-semibold text-gray-900 dark:text-white">
                  {searchQuery || filterStatus !== "all"
                    ...
                    : "No psychologist visit requests"}
                </h3>
                <p className="text-xs sm:text-sm text-muted-foreground mt-1">
                  {searchQuery || filterStatus !== "all"
                    ? "Try adjusting your search or filter criteria."
                    : "No psychologist visit requests have been submitted yet."}
                </p>
              </div>
              {(searchQuery || filterStatus !== "all") && (
                <Button
