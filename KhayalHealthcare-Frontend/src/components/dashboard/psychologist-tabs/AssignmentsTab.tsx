...
interface VisitRequestAssignment {
  _id: string;
  ...
  status: "assigned" | "accepted" | "cancelled" | "in_progress" | "completed";
  created_at: string;
  subscriber: {
    id: string;
    name: string;
...
type ActionType = "accept" | "cancel" | "start" | "complete";
type FilterStatus =
  | "all"
  | "assigned"
  | "accepted"
...
export default function AssignmentsTab() {
  ...
  // Query psychologist assignments
  const { data: assignments = [], isLoading: assignmentsLoading } = useQuery<
    VisitRequestAssignment[]
  >({
    queryKey: ["visit-requests-psychologist-assignments"],
    queryFn: async () => {
      const res = await apiRequest(
        "GET",
        API_ENDPOINTS.VISIT_REQUESTS_PSYCHOLOGIST_ASSIGNMENTS
      );
      return res.json();
    },
    staleTime: 30000,
  ...
  const updateAssignmentStatusMutation = useMutation({
    mutationFn: async ({
      ...
      const res = await apiRequest(
        "PATCH",
        API_ENDPOINTS.VISIT_REQUESTS_PSYCHOLOGIST_STATUS(assignmentId),
        { status }
      );
      return res.json();
    },
    onSuccess: (_, variables) => {
      ...
      const actionText =
        variables.status === "in_progress" ? "started" : variables.status;
      toast({
        title: "Success",
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-6 gap-3 sm:gap-4">
        <Card className="bg-gradient-to-br from-indigo-50 to-indigo-100 dark:from-indigo-900/30 dark:to-indigo-800/20 border-indigo-200 dark:border-indigo-700">
          <CardContent className="p-3 sm:p-4">
            <div className="flex items-center justify-between">
              <div className="min-w-0 flex-1">
                <p className="text-xs sm:text-sm font-medium text-indigo-700 dark:text-indigo-300 truncate">
                  Total
                </p>
                <p className="text-xl sm:text-2xl md:text-3xl font-bold text-indigo-900 dark:text-indigo-100">
                  {assignmentStats.total}
                </p>
              </div>
              <Briefcase className="h-6 w-6 sm:h-8 sm:w-8 md:h-10 md:w-10 text-indigo-600 dark:text-indigo-400 flex-shrink-0" />
            </div>
          </CardContent>
        </Card>

        <Card className="bg-gradient-to-br from-yellow-50 to-yellow-100 dark:from-yellow-900/30 dark:to-yellow-800/20 border-yellow-200 dark:border-yellow-700">
          <CardContent className="p-3 sm:p-4">
            <div className="flex items-center justify-between">
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xl sm:text-2xl md:text-3xl font-bold text-yellow-900 dark:text-yellow-100">
                  {assignmentStats.assigned}
                </p>
              </div>
              <Clock className="h-6 w-6 sm:h-8 sm:w-8 md:h-10 md:w-10 text-yellow-600 dark:text-yellow-400 flex-shrink-0" />
            </div>
          </CardContent>
        </Card>
        ...
        <CardContent className="p-3 sm:p-4 md:p-6">
          <div className="space-y-3 sm:space-y-4">
            <div className="relative">
              ...
              <Input
                placeholder="Search by patient name, phone, address, or description..."
                value={searchQuery}
      ...
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs sm:text-sm text-muted-foreground flex-shrink-0 ml-2">
          ...
          {filteredAssignments.length === 1 ? "session" : "sessions"} found
        </span>
      </div>
            ...
            return (
              <Card
                ...
                className={cn(
                  ...
                  isAssigned && "ring-2 ring-yellow-200 dark:ring-yellow-800",
                ...
                <CardHeader className="pb-2 sm:pb-3 pt-3 sm:pt-4">
                  <div className="space-y-1 sm:space-y-2">
                    <div className="flex items-center gap-2 sm:gap-3 pr-16 sm:pr-24">
                      <div className="p-1.5 sm:p-2 rounded-lg bg-gradient-to-br from-indigo-50 to-indigo-100 dark:from-indigo-900/30 dark:to-indigo-800/20 flex-shrink-0">
                        <Brain className="h-4 w-4 sm:h-5 sm:w-5 md:h-6 md:w-6 text-indigo-600 dark:text-indigo-400" />
                      </div>
                      <div className="min-w-0 flex-1">
                  ...
                  <div className="space-y-2 sm:space-y-3">
                    <div className="p-2 sm:p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                      <div className="space-y-1.5 sm:space-y-2">
                        ...
                        {assignment.subscriber.age && (
                          <div className="flex items-center gap-1.5 sm:gap-2 pt-1">
                            <Info className="h-3 w-3 sm:h-4 sm:w-4 text-primary flex-shrink-0" />
                            <span className="text-xs sm:text-sm text-gray-900 dark:text-white">
                              Age: {assignment.subscriber.age} years
                            </span>
                          </div>
                        )}

                        {assignment.subscriber.previous_illness && (
                          <div className="flex items-start gap-1.5 sm:gap-2">
                            <AlertCircle className="h-3 w-3 sm:h-4 sm:w-4 text-primary flex-shrink-0 mt-0.5" />
                            <span className="text-xs sm:text-sm text-gray-900 dark:text-white line-clamp-2">
                              Previous illness:{" "}
                              {assignment.subscriber.previous_illness}
                            </span>
                      ...
                      {assignment.status === "assigned" && (
                        <div className="flex flex-col sm:flex-row gap-2 w-full">
                          <Button
                            variant="default"
                            size="sm"
                            className="flex-1 bg-green-600 hover:bg-green-700 text-white h-8 sm:h-9 text-xs sm:text-sm"
                            onClick={() =>
                              handleAction(
                                "accept",
                                assignment._id,
              ...
              <div>
                ...
                <p className="text-xs sm:text-sm text-muted-foreground mt-1">
                  {searchQuery || statusFilter !== "all"
                    ...
                    : "You haven't received any psychologist session assignments yet."}
                </p>
              </div>
              {(searchQuery || statusFilter !== "all") && (
                <Button
                    ...
                    <span className="text-indigo-700 dark:text-indigo-300">
                      ...
                      <span className="font-semibold">
                        {dialogState.action === "accept" &&
                          "accept this psychologist session"}
                        {dialogState.action === "start" &&
                          "start this psychologist session"}
                ...
                {dialogState.action === "accept" && (
                  <div className="p-3 bg-green-50 dark:bg-green-900/20 rounded-lg border border-green-200 dark:border-green-800">
                    <div className="flex gap-2">
                      <UserCheck className="h-4 w-4 text-green-600 dark:text-green-400 flex-shrink-0 mt-0.5" />
                      <div className="space-y-1 text-xs">
                        <p className="font-semibold text-green-700 dark:text-green-300">
                          Accepting the assignment:
                        </p>
                        <ul className="text-green-600 dark:text-green-400 space-y-0.5 list-disc list-inside">
                          <li>You commit to conduct this session</li>
                          <li>The patient will be notified</li>
                    ...
                    <div className="flex gap-2">
                      ...
                      <div className="space-y-1 text-xs">
                        ...
                        <ul className="text-purple-600 dark:text-purple-400 space-y-0.5 list-disc list-inside">
                          <li>You are beginning the session now</li>
                    ...
                    <div className="flex gap-2">
                      ...
                      <div className="space-y-1 text-xs">
                        ...
                        <ul className="text-red-600 dark:text-red-400 space-y-0.5 list-disc list-inside">
                          <li>This session will be cancelled</li>
                    ...
                    <div className="flex gap-2">
                      ...
                      <div className="space-y-1 text-xs">
                        ...
                        <ul className="text-blue-600 dark:text-blue-400 space-y-0.5 list-disc list-inside">
                          <li>Confirms the session was conducted</li>
                          <li>Updates your completed sessions count</li>
                          <li>Notifies the patient</li>
                        </ul>
                      </div>
                    </div>
                  </div>
