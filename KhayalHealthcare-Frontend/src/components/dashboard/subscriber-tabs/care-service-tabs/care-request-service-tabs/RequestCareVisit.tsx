...
const insertCareVisitRequestSchema = z.object({
  subscriber_id: z.string(),
  request_type: z.enum(["self", "caretaker"]).default("self"),
  description: z.string().min(1, "Description is required"),
  preferred_date: z.string(),
});

interface RequestCareVisitProps {
  onSuccess?: () => void;
}

export class RequestCareVisitService {
  private user: any;
  ...
  Component = ({}: RequestCareVisitProps = {}) => {
    ...
    const careForm = useForm<z.infer<typeof insertCareVisitRequestSchema>>({
    ...
    const createCareRequestMutation = useMutation({
      mutationFn: async (
        data: z.infer<typeof insertCareVisitRequestSchema>
      ) => {
        const res = await apiRequest(
          "POST",
          API_ENDPOINTS.VISIT_REQUESTS_CARE,
          data
        );
        return res.json();
      },
      onSuccess: () => {
        // Invalidate queries but don't cause navigation
    ...
    const handleConfirmSubmit = (event: React.MouseEvent) => {
      ...
      if (pendingFormData) {
        createCareRequestMutation.mutate(pendingFormData);
      }
    };
    ...
    return (
      <div className="w-full p-3 sm:p-4 md:p-6">
        <div className="w-full max-w-2xl mx-auto space-y-4 sm:space-y-6">
          ...
          <div className="text-center">
            ...
            <p className="text-xs sm:text-sm text-muted-foreground">
              Schedule a visit with our healthcare team
            </p>
          </div>
          ...
          >
            <div className="absolute top-0 left-0 w-full h-0.5 sm:h-1 bg-gradient-to-r from-pink-400 to-rose-500" />
              ...
              <div className="flex items-center gap-2 sm:gap-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <CardDescription className="text-[10px] sm:text-xs text-muted-foreground">
                    Provide your preferred date and any additional information
                  </CardDescription>
                </div>
              </div>
            </CardHeader>
              ...
              <Form {...careForm}>
                <form
                  onSubmit={(e) => {
                    e.preventDefault();
                    e.stopPropagation();
                    careForm.handleSubmit((data) => handleFormSubmit(e, data))(
                      e
                    );
                  }}
                ...
                >
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Textarea
                            placeholder="Describe your health concerns or any specific needs..."
                            rows={4}
                  ...
                  <Button
                    ...
                    className={cn(
                      ...
                      "disabled:from-pink-400 disabled:to-rose-400",
                ...
                <div className="p-2 sm:p-3 bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
                  <div className="flex items-start gap-1.5 sm:gap-2">
                    ...
                    <p className="text-[10px] sm:text-xs text-blue-700 dark:text-blue-300 leading-relaxed">
                      A care coordinator will contact you within 24 hours to
        ...
        <Dialog
          ...
          onOpenChange={(open) => {
            if (!createCareRequestMutation.isPending) {
          ...
          <DialogContent className="w-[95vw] max-w-md mx-auto rounded-lg">
            <DialogHeader className="text-center sm:text-left">
              ...
              <DialogDescription className="text-xs sm:text-sm text-gray-600 mt-2">
                Please review your care visit details before submitting.
              </DialogDescription>
            </DialogHeader>

            {pendingFormData && (
              <div className="py-3 sm:py-4 space-y-3 sm:space-y-4">
                <div className="bg-gradient-to-br from-pink-50 to-pink-100 dark:from-pink-900/20 dark:to-pink-800/20 p-3 sm:p-4 rounded-lg border border-pink-200 dark:border-pink-800">
                  <h4 className="font-medium text-pink-900 dark:text-pink-100 mb-1.5 sm:mb-2 flex items-center gap-1.5 sm:gap-2 text-xs sm:text-sm">
                    <CalendarDays className="h-3 w-3 sm:h-4 sm:w-4" />
                    Preferred Date & Time
                  </h4>
                  <p className="text-pink-800 dark:text-pink-200 text-xs sm:text-sm">
                ...
                <div className="bg-amber-50 dark:bg-amber-900/20 p-2.5 sm:p-3 rounded-lg border border-amber-200 dark:border-amber-800">
                  <div className="flex items-start gap-1.5 sm:gap-2">
                    ...
                    <p className="text-amber-800 dark:text-amber-200 text-[10px] sm:text-xs">
                      ...
                      will be sent to our care team for review and scheduling.
                    </p>
                  </div>
                </div>
              </div>
