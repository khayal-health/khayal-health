...
type VisitRequest = {
  ...
  request_type: "self" | "psychologist";
  description: string;
...
export class CounselingRequestsListService {
  private user: any;
  ...
  Component = () => {
    ...
    // Query counseling requests
    const {
      data: psychologistVisitRequests = [],
      isLoading: psychRequestsLoading,
    } = useQuery<VisitRequest[]>({
      queryKey: [
        API_ENDPOINTS.VISIT_REQUESTS_PSYCHOLOGIST_SUBSCRIBER(this.user!._id),
      ],
      queryFn: getQueryFn({ on401: "returnNull" }),
      select: (data) => {
        if (!data || !Array.isArray(data)) return [];
        return data.filter((request) => request && typeof request === "object");
      },
      staleTime: 30000,
      gcTime: 300000,
    ...
    const requestStats = useMemo(() => {
      ...
      return psychologistVisitRequests.reduce(
        (acc, request) => {
          acc.total++;
    ...
    const filteredRequests = useMemo(() => {
      if (!psychologistVisitRequests) return [];

      let filtered = psychologistVisitRequests;
    ...
    }, [psychologistVisitRequests, statusFilter, searchQuery]);
          ...
          <CardContent className="p-3 sm:p-4 md:p-6">
            <div className="space-y-3 sm:space-y-4">
              <div className="relative">
                ...
                <Input
                  placeholder="Search by psychologist name, description, or request ID..."
                  value={searchQuery}
                          ...
                          {request.psychologist?.name && (
                            <div className="flex items-center gap-1.5 sm:gap-2">
                              ...
                              <span className="text-xs sm:text-sm text-gray-900 dark:text-white">
                                Psychologist: {request.psychologist.name}
                              </span>
                            </div>
                          )}
                ...
                <div>
                  ...
                  <p className="text-xs sm:text-sm text-muted-foreground mt-1">
                    {searchQuery || statusFilter !== "all"
                      ...
                      : "You haven't submitted any counseling session requests yet."}
                  </p>
                </div>
                {(searchQuery || statusFilter !== "all") && (
                  <Button
